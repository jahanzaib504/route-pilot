...
from .services.generate_eld_logs import generate_eld_logs, ELDComplianceError
        ...
        return Response(
            {
                ...
                "available_hours": 70 - data['cycle_hour']
            },
            status=status.HTTP_400_BAD_REQUEST
        )

    try:
        eld_logs = generate_eld_logs(estimated_time_in_hours, data['current_time'])
    except ELDComplianceError as e:
        return Response(
            {"error": "Could not generate a compliant ELD log for this trip.", "details": str(e)},
            status=status.HTTP_400_BAD_REQUEST
        )

    route_data = {'mapData': map_data, 'eldLogs': eld_logs}
